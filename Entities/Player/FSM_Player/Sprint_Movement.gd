...
const MAX_SPRINT_TIME : float = 180.0
...
func _enter(_player : State_Controller) -> void:
	...
	sprint_time = clampf(sprint_time - (current_time - exit_time), 0.0, MAX_SPRINT_TIME)
...
func _update(player : State_Controller, delta : float) -> void:
	...
	sprint_time = clampf(sprint_time + delta, 0.0, MAX_SPRINT_TIME)
